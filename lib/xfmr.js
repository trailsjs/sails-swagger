...
const Transformer = {
    ...
    getPaths(sails) {
        ...
        return _.union(inferredPaths ,  Transformer.getDefinitionsFromRouteConfig(sails)  );
    },
                ...
                swaggerdef.responses = _.chain(swaggerdef.responses || {})
                    .map(function(response, responseCode) {
                        if (response.schema || response.model)
                            response.schema = '#/definitions/' + (response.schema || response.model);
                        return response;
                    }).value();
                swaggerdef.parameters = _.chain(swaggerdef.parameters || [])
                    .map(function(parameter) {
                        return typeof parameter == 'string' ? '#/definitions/' + parameter : parameter;
                    }).value();
